...
async function incrementViews(portfolioId) {
  try {
    await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:5001'}/api/portfolio-stats/increment-views`, {
      method: 'POST',
...
function ContactForm({ content, styles, portfolio }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:5001'}/api/contact/send`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          name: formData.name,
